    ...
    return (
      <SectionWrapper>
        <GlassCard className="text-center py-20">
          ...
          <button
            className="rounded-xl bg-cyan-500 px-6 py-3 text-sm font-semibold text-slate-950 hover:bg-cyan-400 transition"
            ...
            {top_contributing_features.map((feature, index) => (
              <div key={`${feature.feature}-${index}`}>
                ...
                <div className="h-2 rounded-full bg-white/10">
                  <div className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-purple-500" style={{ width: `${Math.min(Number(feature.impact), 100)}%` }} />
                </div>
              </div>
            ))}
          </div>
